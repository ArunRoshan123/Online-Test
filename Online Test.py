...
def exam():
    ...
    q1 = radio("Q1. What is the currency of saudi?",['Rupee','Dollar','Pound','Riyal'])
    if q1 =='Riyal':
        c+=1

    q2 = radio("Q2. Which is biggest beach in India?",['Rock beach','Mahabalipuram','Marina','Baga beach'])
    if q2 =='Marina':
        c+=1

    q3 = radio("Q3. what is the capital city of france",['los angels','calfornia','paris','berlin'])
    if q3 =='paris':
        c+=1
    ...
    if c>3:
        ...
        popup("Result", content=message, size='large', implicit_close=True, closable=True)
            ...
            message = [style(put_html("<h1 style='display:inline;border-bottom:0px'>Oops! " + "</h1>" + name + ", your score is <b>"+ str(c) + "</b><br><br>"),'color:red'), style(put_html("<p>Result : <b>FAILED</b></p>"), 'color:red') , put_html("<b>Thank You for your participation.</b><br><br>"), style(put_link('Retry ↺',""), 'color:red;align-content: center;border-radius: 5px;color:#f9faf8;padding: 5px 100px;text-align:center;align-items : center;background-color: white;\background-image: linear-gradient(270deg, #8cf5f5 1%, #0a43f3 100%);')] 
            popup("Result", content=message, size='large', implicit_close=True, closable=True)
